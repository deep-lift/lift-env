...
class MADQN(nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm = nn.LSTM(agent_observation_size, hidden_size, num_layers, batch_first=True, bidirectional=True)
    ...
    def forward(self, x):  # 4 * 46
        h0 = torch.zeros(num_layers * 2, x.size(0), hidden_size).to(device).double()  # 2 for bidirection
        c0 = torch.zeros(num_layers * 2, x.size(0), hidden_size).to(device).double()
        assert x.shape[1] == n_agent * agent_observation_size
        x = x.reshape(-1, n_agent, agent_observation_size)
        x, _ = self.lstm(x, (h0,c0))
        x = x.reshape(-1, x.shape[-1])
        x = F.relu(self.dense1(x))
        x = F.relu(self.dense2(x))
        x = self.dense3(x)
        x = x.reshape(-1, n_agent, action_size)
        return x
...
policy_net = MADQN().to(device).double()
target_net = MADQN().to(device).double()
...
def select_action(state):
    ...
    if sample > eps_threshold:
        ...
        # print('random')
        return torch.tensor([[random.randrange(action_size)] for i in range(n_agent)], device=device, dtype=torch.long)
...
def optimize_model(episode):
    ...
    non_final_mask = torch.tensor(tuple(map(lambda s: s is not None,
                                          batch.next_state)), device=device, dtype=torch.bool).repeat(n_agent).to(device)
    non_final_next_states = torch.cat([s for s in batch.next_state if s is not None]).to(device).double()
    state_batch = torch.cat(batch.state).to(device).double()
    action_batch = torch.tensor(np.stack(batch.action)).to(device)
    reward_batch = torch.tensor(np.stack(batch.reward)).to(device).repeat(n_agent).double()
    ...
    next_state_values = torch.zeros(BATCH_SIZE*n_agent, device=device, dtype=torch.float32).double()
    next_state_values[non_final_mask] = target_net(non_final_next_states).max(2)[0].reshape(-1).detach()
...
if __name__ == '__main__':
    ...
    for episode in range(run_episode + test_episode):
        ...
        while not done:
            step += 1
            s = torch.tensor(state[0]).unsqueeze(0).to(device).double()
            a = select_action(s)
            needed_action_agents = [l_agents.index(i) for i in env_info.agents]
            env_info = env.step(a[needed_action_agents])[default_brain]
            ...
            if not done:
                ...
                ns = torch.tensor(ns[0], dtype=torch.double).unsqueeze(0).to(device)
            else:
                next_state = None
