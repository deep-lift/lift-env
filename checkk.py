...
if __name__ == '__main__':
    ...
    for episode in range(run_episode + test_episode):
        ...
        print('observation : {}'.format(state.shape))
        episode_rewards = 0
        done = False
        ...
        while not done:
            ...
            action = [1] * state.shape[0]
            env_info = env.step(action)[default_brain]
            next_state = env_info.vector_observations
            print('observation : {}'.format(next_state.shape))

            # episode 돌릴때 4997번 스탭일 경우 observation넘어오지 않고 reward도 없는데 local_done도 종료여부가 없음.
